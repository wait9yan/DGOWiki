...
(function () {
	...
	var regex = {
		codeMarkup: /(```*?```)/gm,
		commentReplaceMarkup: new RegExp(
			"\x3c!-- ".concat(commentReplaceMark, " (.*) --\x3e")
		),
		panelWrapperMarkup:
	...
	function renderPanelsStage1(content) {
		...
		var codeBlockMarkers = codeBlockMatch.map(function (item, i) {
			var codeMarker = "\x3c!-- "
				.concat(commentReplaceMark, " CODEBLOCK")
				.concat(i, " --\x3e");
		...
		var panelMatch;
		while (
			(panelWrapperMatch = regex.panelWrapperMarkup.exec(content)) !== null
		) {
			...
			if (hasPanel) {
				panelStartReplacement = "\x3c!-- "
					.concat(commentReplaceMark, ' <div class="')
					.concat([classNames.panelWrapper].join(" "), '"> --\x3e');
				panelEndReplacement = "\n"
					.concat(panelWrapperIndent, "\x3c!-- ")
					.concat(commentReplaceMark, " </div> --\x3e");
				while ((panelMatch = regex.panelMarkup.exec(panelWrapper)) !== null) {
					...
					panelWrapper = panelWrapper.replace(
						panelMatch[0],
						[
							"\n"
								.concat(panelWrapperIndent, "\x3c!-- ")
								.concat(commentReplaceMark, ' <div class="')
								.concat(
									[classNames.panelContainer, panelName].join(" "),
									'"> --\x3e'
								),
